...
const BatchRequest = async (testCases: ITestCase[], source_code: string, language_id: string) => {
    try {
        ...
        const { data } = await axios.request({
            method: 'POST',
            url: `http://${process.env.JUDGE0_HOST}:${process.env.JUDGE0_PORT}/submissions/batch`,
            headers: {
                'X-RapidAPI-Host': 'judge0-ce.p.rapidapi.com',
                'X-RapidAPI-Key': '7afdf2c5eamshf417da207f1ebb4p18e859jsne31b15c5c7ab'
            },
...
const fetchURL = async (tokens) => await axios.request({
    method: 'GET',
    url: `http://${process.env.JUDGE0_HOST}:${process.env.JUDGE0_PORT}/submissions/batch?tokens=${tokens}&base64_encoded=false&fields=stdout,stderr,status_id,language_id,stdin,expected_output,message,status,token,time`,
    headers: {
        'X-RapidAPI-Host': 'judge0-ce.p.rapidapi.com',
        'X-RapidAPI-Key': '7afdf2c5eamshf417da207f1ebb4p18e859jsne31b15c5c7ab'
    }
